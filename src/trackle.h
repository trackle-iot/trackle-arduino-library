...
private:
    ...
    bool sendPublish(const char *eventName, const char *data, int ttl, Event_Type eventType,
                     Event_Flags eventFlag, string msg_id);
    bool registerEvent(const char *eventName, Subscription_Scope_Type eventScope,
                       const char *deviceID);
    bool addSubscription(const char *eventName, EventHandler handler, void *handlerData,
...
public:
    ...
    bool post(const char *funcKey, user_function_int_char_t *func,
              Function_PermissionDef permission = ALL_USERS);

    bool publish(const char *eventName, const char *data, int ttl = DEFAULT_TTL,
                 Event_Type eventType = PUBLIC, Event_Flags eventFlag = EMPTY_FLAGS,
                 string msg_id = "");
    bool publish(string eventName, const char *data, int ttl = DEFAULT_TTL,
                 Event_Type eventType = PUBLIC, Event_Flags eventFlag = EMPTY_FLAGS,
                 string msg_id = "");
    bool publish(const char *eventName, const char *data, Event_Type eventType,
                 Event_Flags eventFlag = EMPTY_FLAGS, string msg_id = "");
    bool publish(string eventName, const char *data, Event_Type eventType,
                 Event_Flags eventFlag = EMPTY_FLAGS, string msg_id = "");
    ...
    void disconnect();
    void loop();
    void setKeepalive(uint32_t interval);
